...
addProperty(module, 'xml_json_helpers', '../util/xml-json-helpers', modules);
...
function ConfigFile_load() {
    ...
    if (ext == '.json' ) {
        self.type = 'json';
        self.data = modules.xml_json_helpers.parseElementtreeSync(filepath);
    } else if (ext == '.xml' || ext == '.appxmanifest') {
        self.type = 'xml';
...
ConfigFile.prototype.save = function ConfigFile_save() {
    var self = this;
    if (self.type === 'json') {
        modules.xml_json_helpers.writeJsonFromXml(self.filepath, self.data)
...
function resolveConfigFilePath(project_dir, platform, file) {
    ...
    // special-case nodekit.json target that is just "nodekit.json". This should be resolved to the real location of the file.
    // TODO: move the logic that contains the locations of nodekit.json from nodekit CLI into plugman.
    if (file == 'nodekit.json') {
        if (platform == 'ubuntu') {
            filepath = path.join(project_dir, 'nodekit.json');
        } else if (platform == 'ios') {
            var iospath = getIOSProjectname(project_dir);
            filepath = path.join(project_dir,iospath, 'nodekit.json');
        } else if (platform == 'android') {
            filepath = path.join(project_dir, 'res', 'raw', 'nodekit.json');
        } else {
            matches = modules.glob.sync(path.join(project_dir, '**', 'nodekit.json'));
            if (matches.length) filepath = matches[0];
        }
        return filepath;
    }
    ...
    if(platform === 'android' && !fs.existsSync(filepath)) {
      filepath = path.join(project_dir, 'app', 'src', 'main', 'res', 'raw', 'nodekit.json');
    }

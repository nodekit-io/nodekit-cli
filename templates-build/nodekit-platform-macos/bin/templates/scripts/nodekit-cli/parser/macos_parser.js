...
function macos_parser(project) {

    try {
        var xcodeproj_dir = fs.readdirSync(project).filter(function(e) { return e.match(/\.xcodeproj$/i); })[0];
        if (!xcodeproj_dir) throw new NodeKitError('The provided path "' + project + '" is not a NodeKit macOS project.');

        // Call the base class constructor
        Parser.call(this, 'macos', project);
        ...
        this.nodekitproj = path.join(project, 'src');
    } catch(e) {
        console.error(e);
        throw new NodeKitError('The provided path "'+project+'" is not a NodeKit macOS project.');
    }
...
require('util').inherits(macos_parser, ios_parser);

module.exports = macos_parser;

// Returns a promise.
macos_parser.prototype.update_from_config = function(config) {
    ...
    events.emit('verbose', 'Wrote out macOS Bundle Identifier to "' + pkg + '"');
    events.emit('verbose', 'Wrote out macOS Bundle Version to "' + version + '"');

    // Update icons
    var icons = config.getIcons('macos');
    ...
    return this.update_build_settings(config).then(function() {
        if (name == parser.originalName) {
            events.emit('verbose', 'macOS Product Name has not changed (still "' + parser.originalName + '")');
            return Q();
        }
        ...
        events.emit('verbose', 'Wrote out macOS Product Name and updated XCode project file names from "'+old_name+'" to "' + name + '".');

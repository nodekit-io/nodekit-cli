...
var PluginInfo = require('nodekit-scripts').common.PluginInfo;
    ...
    engines.forEach(function(engine) {
        ...
        if (defaultEngines[theName]) {
            ...
            if(defaultPlatformIndex > -1 || defaultEngines[theName].platform === '*'){
                ...
                if(theName==='nodekit-scripts-platform-' + platform) nodekitPlatformEngineIndex = uncheckedEngines.length;
...
function tryFetchDependency(dep, install, options) {
    ...
    if ( dep.url == '.' ) {
        ...
        if (fetchdata.source.type === 'local') {
            ...
            // $md5 = md5(uri)
            // Need a Hash(uri) --> $tmpDir/nodekit-scripts-fetch/git-hostname.com-$md5/

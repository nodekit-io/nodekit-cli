...
var nodekit_util = require('./util'),
    ...
    serve        = require('nodekit-scripts').['nodekit-scripts-serve'];
...
function handleRoot(request, response, next) {
    ...
    Object.keys(platforms).forEach(function (platform) {
        if (installedPlatforms.indexOf(platform) >= 0) {
            response.write('<li><a href="' + platform + '/app/' + contentSrc + '">' + platform + '</a></li>\n');
        } else {
            ...
            case '/' + platform + '/project.json':
                response.send({
                    ...
                    'appFileList': shell.find(appDir)
                        ...
                        .map(function (a) { return {'path': a.slice(appDir.length), 'etag': '' + calculateMd5(a)}; })
                });
                break;
...
// so that an URL is resolved against that platform app directory.
function getAbsolutePathHandler() {
    return function (request, response, next) {
        ...
        if (installedPlatforms.indexOf(platform) >= 0 &&
            ...
            response.redirect('/' + platform + '/app' + request.originalUrl);
        } else {
            next();
        }
    };

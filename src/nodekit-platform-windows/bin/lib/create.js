...
module.exports.create = function (destinationDir, config, options) {
    ...
    shell.cp('-r', path.join(root, 'node_modules'), path.join(projectPath, 'nodekit-cli'));

    // copy check_reqs module to nodekit directory
    shell.cp('-rf', path.join(root, 'bin', 'check_reqs*'), path.join(projectPath, 'nodekit-cli'));
    shell.cp('-rf', path.join(root, 'bin', 'lib', 'check_reqs*'), path.join(projectPath, 'nodekit-cli', 'lib'));

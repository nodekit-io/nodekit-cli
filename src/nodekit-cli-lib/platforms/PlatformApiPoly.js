...
var xmlHelpers = require('nodekit-cli')['nodekit-cli-common'].xmlHelpers;
var knownPlatforms = require('./platforms');
var NodeKitError = require('nodekit-cli')['nodekit-cli-common'].NodeKitError;
var PluginInfo = require('nodekit-cli')['nodekit-cli-common'].PluginInfo;
var ConfigParser = require('nodekit-cli')['nodekit-cli-common'].ConfigParser;
var PlatformJson = require('nodekit-cli')['nodekit-cli-common'].PlatformJson;
var ActionStack = require('nodekit-cli')['nodekit-cli-common'].ActionStack;
...
function PlatformApiPoly(platform, platformRootDir, events) {
    ...
    this.events = events || require('nodekit-cli')['nodekit-cli-common'].events;
...
/**
 * Updates installed platform with provided app assets and new app
 *   configuration. This method is required for CLI workflow and will be called
 *   each time before build, so the changes, made to app configuration and app
 *   code, will be applied to platform.
 *
 * @param {NodeKitProject} nodekitProject A NodeKitProject instance, that defines a
 *   project structure and configuration, that should be applied to platform
 *   (contains project's app location and ConfigParser instance for project's
 *   config).
 *
...
PlatformApiPoly.prototype.prepare = function (nodekitProject, options) {
    ...
    var defaultConfig = path.join(this.root, 'nodekit-cli', 'defaults.xml');
    ...
    this._parser.update_app(nodekitProject.locations.app);
...
/**
 * Installs a new plugin into platform. This method only copies non-app files
 *   (sources, libs, etc.) to platform. It also doesn't resolves the
 *   dependencies of plugin. Both of handling of app files, such as assets and
...
PlatformApiPoly.prototype.addPlugin = function (plugin, installOptions) {
    ...
    .then(function () {
        ...
        var targetDir = installOptions.usePlatformApp ?
            self.getPlatformInfo().locations.platformApp :
            self.getPlatformInfo().locations.app;
...
PlatformApiPoly.prototype.removePlugin = function (plugin, uninstallOptions) {
    ...
    .then(function() {
        ...
        var targetDir = uninstallOptions.usePlatformApp ?
            self.getPlatformInfo().locations.platformApp :
            self.getPlatformInfo().locations.app;
...
PlatformApiPoly.prototype.updatePlugin = function (plugin, updateOptions) {
    ...
    // Set up assets installer to copy asset files into platform_app dir instead of app
    updateOptions = updateOptions || {};
    updateOptions.usePlatformApp = true;
...
PlatformApiPoly.prototype.build = function(buildOptions) {
    var command = path.join(this.root, 'nodekit-cli', 'build');
...
PlatformApiPoly.prototype.run = function(runOptions) {
    var command = path.join(this.root, 'nodekit-cli', 'run');
...
PlatformApiPoly.prototype.clean = function() {
    var cmd = path.join(this.root, 'nodekit-cli', 'clean');
...
PlatformApiPoly.prototype.requirements = function() {
    var modulePath = path.join(this.root, 'nodekit-cli', 'lib', 'check_reqs');
    try {
 ...
 *   Ususally it is either <platform>/app or <platform>/platform_app
 *   directories.
 */
...
PlatformApiPoly.prototype._getInstaller = function(type) {
    ...
    return function (item, plugin_dir, plugin_id, options, project) {
        ...
        var appDest = options.usePlatformApp ?
            self.getPlatformInfo().locations.platformApp :
            self._handler.app_dir(self.root);

        var installerArgs = type === 'asset' ? [appDest] :
            type === 'js-module' ? [plugin_id, appDest]:
...
PlatformApiPoly.prototype._getUninstaller = function(type) {
    ...
    return function (item, plugin_dir, plugin_id, options, project) {
        ...
        var uninstallerArgs = (type === 'asset' || type === 'js-module') ? [appDest, plugin_id] :
            [self.root, plugin_id, options, project];
...
/**
 * Copies nodekit.js itself and nodekit-js source into installed/updated
 *   platform's `platform_app` directory.
 *
...
function copyNodeKitSrc(sourceLib, platformInfo) {
    // Copy the nodekit.js file to platforms/<platform>/platform_app/
    // The app dir is nuked on each prepare so we keep nodekit.js in platform_app
    shell.mkdir('-p', platformInfo.locations.platformApp);
    shell.cp('-f', path.join(platformInfo.locations.app, 'nodekit.js'),
        path.join(platformInfo.locations.platformApp, 'nodekit.js'));

    // Copy nodekit-js-src directory into platform_app directory.
    ...
    if(fs.existsSync(nodekitJsSrcPath)) {
        shell.cp('-rf', nodekitJsSrcPath, platformInfo.locations.platformApp);
    }
}
...
function getPlatformVersion (platformRoot) {
    var versionFile = path.join(platformRoot, 'nodekit-cli/version');

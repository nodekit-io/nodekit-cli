...
var path = require('path'),
    fs   = require('fs'),
    action_stack = require('nodekit-cli')['nodekit-cli-common'].ActionStack,
    ...
    pluginSpec = require('../nodekit-cli/plugin_spec_parser'),
    nodekitUtil = require('../nodekit-cli/util');
...
// possible options: subdir, cli_variables, app_dir
// Returns a promise.
module.exports = function installPlugin(platform, project_dir, id, plugins_dir, options) {
    ...
    plugins_dir = plugins_dir || path.join(project_dir, 'nodekit-cli', 'plugins');
...
// possible options: subdir, cli_variables, app_dir, git_ref, is_top_level
// Returns a promise.
...
function runInstall(actions, platform, project_dir, plugin_dir, plugins_dir, options) {
    ...
    return Q().then(function() {
        ...
        return Q(superspawn.maybeSpawn(path.join(project_dir, 'nodekit-cli', 'version'), [], { chmod: true }));
        ...
        function(){
            ...
            if(projectRoot) {
                ...
                // into platform_app but plugman CLI doesn't allow us to do that, so we set it here
...
function handleInstall(actions, pluginInfo, platform, project_dir, plugins_dir, plugin_dir, filtered_variables, options) {
    ...
    .then (function(result) {
        ...
        if (platform == 'android' &&
            ...
            var buildModule;
            try {
                buildModule = require(path.join(project_dir, 'nodekit-cli', 'lib', 'build'));
            } catch (e) {

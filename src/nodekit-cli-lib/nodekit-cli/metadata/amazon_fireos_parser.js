...
amazon_fireos_parser.prototype.app_dir = function() {
    return path.join(this.path, 'assets', 'app');
};
...
amazon_fireos_parser.prototype.nodekitjs_path = function(libDir) {
    var jsPath = path.join(libDir, 'framework', 'assets', 'app', 'nodekit.js');
...
amazon_fireos_parser.prototype.update_app = function() {
    var projectRoot = util.isNodeKit(this.path);
...
// update the overrides folder into the app folder
...
amazon_fireos_parser.prototype.update_project = function(cfg) {
    var platformApp = path.join(this.path, 'assets');
    try {
    ...
    util.deleteSvnFolders(platformApp);
    return Q();
};

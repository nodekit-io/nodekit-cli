...
var config            = require('./config'),
    ...
    superspawn        = require('nodekit-cli')['nodekit-cli-common'].superspawn,
...
function check(hooksRunner, projectRoot) {
    ...
    .then(function () {
        ...
        Q.all(platforms_on_fs.map(function(p) {
            ...
            .then(function() {
                superspawn.maybeSpawn(path.join(scratch, 'platforms', p, 'nodekit-cli', 'version'), [], { chmod: true })
                .then(function(avail) {
                    if (!avail) {
            ...
            superspawn.maybeSpawn(path.join(projectRoot, 'platforms', p, 'nodekit-cli', 'version'), [], { chmod: true })
            .then(function(v) {
                d_cur.resolve(v || '');
            }).catch(function () {

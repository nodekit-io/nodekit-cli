...
PluginManager.prototype.doOperation = function (operation, plugin, options) {
    ...
    .then(function () {
        ...
        fs.writeFileSync(path.join(self.locations.app, 'nodekit_plugins.js'), metadata, 'utf-8');

        // CB-11022 save plugin metadata to both app and platform_app if options.usePlatformApp is specified
        if (options.usePlatformApp) {
            fs.writeFileSync(path.join(self.locations.platformApp, 'nodekit_plugins.js'), metadata, 'utf-8');
        }
    });
};

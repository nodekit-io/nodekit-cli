...
module.exports = {
    ...
    generateDesktopFile: function(config, dir) {
        ...
        if (config.icon() && fs.existsSync(path.join(dir, '../..', config.icon()))) {
            Utils.cp(path.join(dir, '../..', config.icon()), path.join(dir, 'app'));

            content += '\nIcon=app/' + path.basename(config.icon());
        } else {
            Utils.cp(path.join(dir, 'build', 'default_icon.png'), path.join(dir, 'app'));

            content += '\nIcon=app/default_icon.png';
        }

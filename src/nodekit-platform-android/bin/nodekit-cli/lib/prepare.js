...
var FileUpdater = require('nodekit-cli')['nodekit-cli-common'].FileUpdater;
...
module.exports.clean = function (options) {
    ...
    if ((options && options.noPrepare) || !fs.existsSync(this.locations.nodekitJson) ||
    ...
    return Q().then(function () {
        cleanApp(projectRoot, self.locations);
...
function updateConfigFilesFrom(sourceConfig, configMunger, locations) {
    events.emit('verbose', 'Generating platform-specific nodekit.json from defaults for android at ' + locations.nodekitJson);

    // First cleanup current config and merge project's one into own
    // Overwrite platform nodekit.json with defaults_nodekit.json.
    shell.cp('-f', locations.defaultNodeKitJson, locations.nodekitJson);

    // Then apply config changes from global munge to all config files
    // in project (including project's config)
    configMunger.reapply_global_munge().save_all();

    events.emit('verbose', 'Merging project\'s nodekit.json into platform-specific android nodekit.json');
    // Merge changes from app's nodekit.json into platform's one
    var config = new ConfigParser(locations.nodekitJson);
...
/**
 * Updates platform 'app' directory by replacing it with contents of
 *   'platform_app' and  project app. Also copies project's overrides' folder into
 *   the platform 'app' folder
 *
...
function updateApp(nodekitProject, destinations) {
    ...
    var targetDir = path.relative(nodekitProject.root, destinations.app);
    events.emit(
...
/**
 * Cleans all files from the platform 'app' directory.
 */
function cleanApp(projectRoot, locations) {
    var targetDir = path.relative(projectRoot, locations.app);
...
function default_versionCode(version) {
    ...
    events.emit('verbose', 'android-versionCode not found in nodekit.json. Generating a code based on version in nodekit.json (' + version + '): ' + versionCode);
    return versionCode;
}
...
function updateSplashes(nodekitProject, platformResourcesDir) {
    ...
    // if there are "splash" elements in nodekit.json
...
function updateIcons(nodekitProject, platformResourcesDir) {
    ...
    // if there are icon elements in nodekit.json
    if (icons.length === 0) {
    ...
    // project's nodekit.json location, so we use it as base path.
...
/**
 * Gets and validates 'AndroidLaunchMode' prepference from nodekit.json. Returns
 *   preference value and warns if it doesn't seems to be valid
 *
 * @param   {ConfigParser}  platformConfig  A configParser instance for
 *   platform.
 *
 * @return  {String}                  Preference's value from nodekit.json or

...
function copyJsAndLibrary(projectPath, shared, projectName) {
    var nestedNKNodeKitPath = getFrameworkDir(projectPath, false);
    var srcNodeKitJsPath = path.join(ROOT, 'bin', 'templates', 'project', 'assets', 'app', 'nodekit.js');
    shell.cp('-f', srcNodeKitJsPath, path.join(projectPath, 'assets', 'app', 'nodekit.js'));
    ...
    shell.mkdir('-p', path.join(projectPath, 'platform_app'));
    shell.cp('-f', srcNodeKitJsPath, path.join(projectPath, 'platform_app'));
...
function writeProjectProperties(projectPath, target_api) {
    ...
    subProjects = subProjects.filter(function(p) {
        return !(/^NKNodeKit$/m.exec(p) ||
                 /[\\\/]nodekit-android[\\\/]framework$/m.exec(p) ||
                 /^(\.\.[\\\/])+framework$/m.exec(p)
                 );
    });
    subProjects.unshift('NKNodeKit');
...
function prepBuildFiles(projectPath) {
    var buildModule = require(path.resolve(projectPath, 'nodekit-cli/lib/builders/builders'));
 ...
 *   and NKNodeKit framework will be symlinked to created application.
...
exports.create = function(project_path, config, options, events) {
    ...
    project_path = path.relative(process.cwd(), (project_path || 'NodeKitExample'));
    ...
    .then(function() {
        ...
        setShellFatal(true, function() {
            ...
            /*NODEKIT*/  shell.mkdir('-p', path.join(project_path, 'assets'));  shell.cp('-r', path.join(project_template_dir, 'assets'), project_path);
            /*NODEKIT*/ shell.mkdir('-p', path.join(project_path, 'res')); shell.cp('-r', path.join(project_template_dir, 'res'),  project_path);
